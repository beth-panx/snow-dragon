...
@PreventIframe("/snowDragonMessageExtension/config.html")
@PreventIframe("/snowDragonMessageExtension/action.html")
export default class SnowDragonMessageExtension implements IMessagingExtensionMiddlewareProcessor {
    ...
    public async onQuerySettingsUrl(context: TurnContext): Promise<{ title: string, value: string }> {
        return Promise.resolve({
            title: "Snow Dragon Message Extension Configuration",
            value: `https://${process.env.HOSTNAME}/snowDragonMessageExtension/config.html`
        });
    }

 ...
 */
interface ISnowDragonConnectorData {
    webhookUrl: string;
    user: string;
    appType: string;
    groupName: string;
...
/**
 * Implementation of the "SnowDragonConnectorConnector" Office 365 Connector
 */
@ConnectorDeclaration(
    "/api/connector/connect",
    "/api/connector/ping"
)
@PreventIframe("/snowDragonConnector/config.html")
export class SnowDragonConnector implements IConnector {
    ...
    public Ping(req: Request): Array<Promise <void>> {
        ...
        try {
            this.connectors.push("/connectors",
                (this.connectors.getData("/connectors") as ISnowDragonConnectorData[]).filter(((c) => {
                    return c.existing;
                })));
        } catch (error) {
        ...
        return (this.connectors.getData("/connectors") as ISnowDragonConnectorData[]).map((connector, index) => {
